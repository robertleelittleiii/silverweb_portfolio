...
function bindClickToPortfolioItem () {
    $('.portfolio-item').click(function(){
        //   console.log(this);
        window.location.href = $($(this)).find("a.portfolio-detail-link").attr('href');

    //   $(this).find("a.portfolio-detail-link").click();
    });
}
...
function setUpOrderChange() {
    ...
    $('#portfolio-block').sortable({
        ...
        update: function(){
            $.ajax({
                type: 'post',
                data: $('#portfolio-block').sortable('serialize') + "&   page=" + currentPage,
                dataType: 'script',
                complete: function(request){
                    $('#portfolio-block').effect('highlight');
                },
                url: '/portfolios/sort'
            })
        }
...
function bindPortfolioMenu()
{
    ...
    $(".edit-portfolio").hover(
        function () {
            $(this).append($("<div class='edit-portfolio-menu'>Edit</br>To Next Page</br></div>"));
        },
        function () {
