...
class ArtifactsController < ApplicationController
  ...
  ACTION_VIEWERS = [['Image Slider', 'artifact_detail'], ['Information Page', 'artifact_detail_custom']].freeze
  ...
  def create_empty_record
    @artifact = Artifact.new
    @artifact.portfolio_id = begin
                              params[:portfolio_id]
                             rescue StandardError
                               nil
                            end
    @artifact.name = 'New Item'.dup
    @artifact.description = 'Enter Description Here.'.dup
    @artifact.position = 99
    @artifact.artifact_action_viewer = 'artifact_detail'.dup
  ...
  def update_checkbox_tag
    @artifact = Artifact.find(params[:id])
    @tag_name = params[:tag_name] || 'tag_list'.dup
    # truely toggle the value

    if @artifact.send(@tag_name).include?(params[:field])
