    ...
    module SiteControllerExtensions
      ...
      module InstanceMethods
        def show_custom_portfolios
          ...
          @customs_per_page = Settings.portfolios_per_page.to_i == 0 ? 8 : Settings.portfolios_per_page.to_i
          ...
          if params[:top_menu] && (@get_first_submenu == 'true')
        ...
        def show_portfolios
          ...
          @portfolios_per_page = Settings.portfolios_per_page.to_i == 0 ? 8 : Settings.portfolios_per_page.to_i
          ...
          puts("@portfolio_ids --> #{@portfolio_ids.inspect}")
        ...
        def show_artifact_group
          ...
          @artifacts_per_page = Settings.portfolios_per_page.to_i == 0 ? 8 : Settings.portfolios_per_page.to_i
          @category_id = params[:category_id] || ''
          @department_id = params[:department_id] || ''
          @category_children = params[:category_children] || false
          @get_first_submenu = params[:get_first_sub] || false
          ...
          # @page_name=Menu.find(session[:parent_menu_id]).name rescue ""
          # begin
          if @category_children == 'true'
            @categories =  create_menu_lowest_child_list(@category_id, nil, false) + [@category_id]
            puts("categories: #{@categories.inspect} ")
            @artifacts_list = Artifact.where(artifact_active: true).tagged_with(@categories, any: true, on: :category).tagged_with(@department_id, on: :department)

          else
            if @category_id.blank? && @department_id.blank?
              puts('both  @category_id and  @department_id blank!')
              @artifacts_list = Artifact.where(artifact_active: true)
            else
              if @category_id.empty? || (@category_id = @department_id)
                puts('@category_id blank!')
        ...
        def show_artifact_custom_group
          ...
          @artifact_first = params[:page].blank? ? '1'.dup : (params[:page].to_i * @artifacts_per_page - (@artifacts_per_page - 1))
        ...
        def artifact_detail
          ...
          respond_to do |format|
            if @artifact.action_viewer != 'artifact_detail'
              redirect_to action: @artifact.action_viewer, params: params.permit(params.keys).to_h
              return
            else
              format.html { render action: @page_template } # show.html.erb
              format.xml  { render xml: @page }
